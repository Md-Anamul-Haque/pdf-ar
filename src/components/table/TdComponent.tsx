...
const TdComponent = ({ className, tdData }: { className: string; tdData: TdType }) => {
    if (typeof tdData == 'object') {
        ...
        return (
            <Td colSpan={colSpan} text-align={td_th_textAlign(tdData)} rowSpan={rowSpan} className={`${className} ${className2}`} key={uuidv4()}>{tdData.value}</Td>
        )
    } else {
        return <Td className={className} text-align={td_th_textAlign(tdData)} key={uuidv4()}>{tdData}</Td>
